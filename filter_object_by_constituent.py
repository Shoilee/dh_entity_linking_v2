...
def run_query(file, new_graph):
    ...
    WHERE {
        ?event ?prop <https://hdl.handle.net/20.500.11840/pi57757>. 
         ?event crm:P24_transferred_title_of ?object .
         ?object a crm:E22_Human-Made_Object ;
             (a|!a)+ ?child . 
         ?child ?childProp ?childPropVal.
         ?someSubj ?incomingChildProp ?child. 
...
def run(directory="/Users/sarah_shoilee/Desktop/Sarah/Bronbeek_Data/linktonmvw"):
    ...
    try:
        ...
        new_graph.serialize(destination=os.path.join(directory, "filtered_obj.ttl"))
        # show dataframe
        print(log_table.to_string())
        log_table.to_csv(path_or_buf=os.path.join(directory, "object_filter_log.csv"), sep=',')
